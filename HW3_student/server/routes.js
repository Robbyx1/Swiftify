...
const song = async function (req, res) {
  ...
  LIMIT 1
`;
  connection.query(query, [songId], (err, data) => {
    if (err || data.length === 0) {
      console.log(err);
      res.json({});
    } else {
...
const albums = async function (req, res) {
  // SQL query to select all columns from the Albums table and order by release_date descending
  const query = `
    SELECT *
    FROM Albums
...
const top_songs = async function (req, res) {
  ...
  `;
    ...
    connection.query(paginatedQuery, [pageSize, offset], (err, data) => {
      if (err) {
        console.error('Error fetching top songs with pagination:', err);
        res.status(500).json({ error: 'Internal Server Error' });
      } else {
        res.json(data);
      }
...
const top_albums = async function (req, res) {
  ...
  const queryParams = page ? [pageSize, offset] : [];
...
const search_songs = async function (req, res) {
  ...
  let queryParams = [
    `%${title}%`, // For substring matching in title
  ...
  // Handle the explicit filter
  if (explicit !== 'true') {
    query += `AND explicit = FALSE`;
  }
